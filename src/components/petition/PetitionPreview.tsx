...
export function PetitionPreview({ petitionData }: { petitionData: PetitionData }) {
  ...
  const renderSignatureLines = () => {
    ...
    for (let i = 0; i < Math.min(petitionData.signatureCount, 50); i++) {
      lines.push(
        <TableRow key={i}>
          <TableCell className="border border-black p-2 w-[15%] align-top">
            {i+1}. __/__/20__
          </TableCell>
          <TableCell className="border border-black p-2 w-[35%]">
            <div className="h-8"></div>
            <div className="text-xs text-center border-t border-dashed pt-1">Printed Name</div>
          </TableCell>
  ...
  return (
    <div className="space-y-4 p-4 bg-white">
      ...
      <div className="text-sm leading-relaxed">
        ...
        <span className="mx-1 px-2 inline-block min-w-[150px]">{petitionData.party || "________________"}</span> 
        Party and entitled to vote at the next primary election of such party, to be held on 
        <span className="mx-1 px-2 inline-block min-w-[150px]">{petitionData.electionDate || "________________"}</span>, 
        {petitionData.electionYear || "20__"}; that my place of residence is truly stated opposite my signature hereto, and I do hereby designate the following named person (or persons) as a candidate (or candidates) for the nomination of such party for public office or for election to a party position of such party.
      </div>
      ...
      <div className="border border-black">
        <Table className="border-collapse w-full m-0 p-0">
          <TableHead>
            <TableRow>
              <TableHeader className="border border-black font-bold text-left p-2 w-[30%] text-black bg-white">
                Name(s) of Candidate(s)
              </TableHeader>
              <TableHeader className="border border-black font-bold text-left p-2 w-[35%] text-black bg-white">
      ...
      <div className="border border-black">
        <Table className="border-collapse w-full m-0 p-0">
          <TableHead>
            <TableRow>
              <TableHeader className="border border-black font-bold text-left p-2 w-[15%] text-black bg-white">
                Date
              </TableHeader>
              <TableHeader className="border border-black font-bold text-left p-2 w-[35%] text-black bg-white">
                Name of Signer
      ...
      {/* Hidden on preview but included in PDF: Witness statement section */}
